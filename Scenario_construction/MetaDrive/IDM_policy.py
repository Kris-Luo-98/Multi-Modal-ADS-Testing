...
def run_scenario(road_network, env_info, actors, road_type, ID, folder_path, seq):
    ...
    try:
        ...
        env.engine.add_policy(npc.id, IDMPolicy, npc, env.engine.generate_seed())
        # Build NPC 2
        if len(car_dict) == 3:
            ...
            env.engine.add_policy(npc_1.id, IDMPolicy, npc_1, env.engine.generate_seed())
...
def main():
    ...
    result_folder = f".\\IDMPolicy\\results_{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}"
